...
def compute_acc_F1(pred, target, info='', _type='action'):
    ...
    recall_action = recall_score(target, pred)
    precision = precision_score(target, pred)
    result[_type+'_accuracy'] = acc_action
    result[_type+'_f1'] = f1_action
    result[_type+'_recall'] = recall_action
    result[_type+'_precision'] = precision
    info += 'Acc: {:.4f}; F1: {:.4f}; Recall: {:.4f}; Prec: {:.4f}; '.format(acc_action, f1_action, recall_action, precision)
    return result, info
