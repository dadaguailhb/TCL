...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, n_frames=16, embed_dim=768, depth=12,
        ...
        self.classifier1 = nn.Linear(embed_dim, 1)
        self.classifier2 = nn.Linear(embed_dim, 7) # num_action
    ...
    def forward(self, x: Tensor, seq_mean_pool: bool) -> Tensor:
        assert len(x.shape) == 5, "x must be 5D"
        ...
        x = self.norm(x)
        x = self.conv(x)
        score1 = self.classifier1(x)
        score2 = self.classifier2(x)
